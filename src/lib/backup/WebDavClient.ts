...
export class WebDavClient {
    ...
    constructor(config: WebDavConfig) {
        ...
        console.log(`[WebDav] Username chars: ${config.username.split('').map(c => c.charCodeAt(0)).join(',')}`);
     ...
     * Upload a file
     */
    async uploadFile(path: string, content: string, contentType: string = 'application/json'): Promise<void> {
        const url = this.getUrl(path);
