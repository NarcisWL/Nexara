...
import { ChevronLeft, Save, Sparkles, Download, Trash2, Check, Upload, Database, ChevronRight } from 'lucide-react-native';
...
export default function SuperAssistantSettingsScreen() {
    ...
    const { getSession, updateSessionTitle, updateSessionInferenceParams, generateSessionTitle, deleteSession } = useChatStore();
            ...
            >
                ...
                >
                    ...
                    <Typography variant="label" className="text-gray-400 font-bold uppercase text-[10px] tracking-widest mb-3">
                        <Sparkles size={10} color="#64748b" className="mr-1" /> {t.superAssistant.globalKnowledge}
                    </Typography>
                    <View className="bg-gradient-to-br from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20 rounded-3xl p-5 border border-purple-200 dark:border-purple-800/30 mb-8">
                        ...
                        <View className="bg-white/50 dark:bg-black/20 rounded-2xl p-4 space-y-2">
                            <View className="flex-row items-center justify-between">
                                <Typography className="text-sm text-gray-700 dark:text-gray-300">📄 {t.superAssistant.docCount}</Typography>
                                <Typography className="text-sm font-bold text-gray-900 dark:text-white">
                                    {stats?.totalDocuments || 0}
                                </Typography>
                            </View>
                            <View className="flex-row items-center justify-between">
                                <Typography className="text-sm text-gray-700 dark:text-gray-300">💬 {t.superAssistant.sessionMemory}</Typography>
                                <Typography className="text-sm font-bold text-gray-900 dark:text-white">
                                    {stats?.totalSessions || 0}
                                </Typography>
                            </View>
                            <View className="flex-row items-center justify-between">
                                <Typography className="text-sm text-gray-700 dark:text-gray-300">🔢 {t.superAssistant.totalVectors}</Typography>
                                <Typography className="text-sm font-bold text-gray-900 dark:text-white">
                    ...
                    >
                        {isExporting ? (
                            ...
                            <>
                                ...
                                <Typography className="text-indigo-600 dark:text-indigo-400 font-bold">{t.superAssistant.exportHistory}</Typography>
                            </>
                        )}
                    </TouchableOpacity>
                    ...
                    <Typography variant="label" className="text-gray-400 font-bold uppercase text-[10px] tracking-widest mb-3">
                        <Sliders size={10} color="#64748b" className="mr-1" /> {t.conversation.inferenceSettings}
                    </Typography>
                        ...
                        >
                            {isGeneratingTitle ? (
                                ...
                                <>
                                    ...
                                    <Typography className="text-indigo-600 text-[10px] font-bold">{t.superAssistant.aiGenerate}</Typography>
                                </>
                            )}
                        </TouchableOpacity>
                    </View>
                    ...
                    <View className="bg-gray-50 dark:bg-zinc-900 rounded-3xl p-5 border border-gray-100 dark:border-zinc-800 mb-8">
                        {/* Icon Type */}
                        <Typography className="text-base font-bold text-gray-900 dark:text-gray-100 mb-3">{t.superAssistant.iconStyle}</Typography>
                        <View className="flex-row flex-wrap gap-2 mb-4">
                            {PRESET_FAB_ICONS.map((preset) => (
                                <TouchableOpacity
                                    ...
                                    className={`flex-1 min-w-[45%] p-3 rounded-xl ${preferences.fab.iconType === preset.type
                                ...
                                >
                                    <View className="flex-row items-center">
                                        ...
                                        <Typography className={`ml-2 text-sm font-semibold ${preferences.fab.iconType === preset.type ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-600 dark:text-gray-400'
                                            }`}>
                            ...
                            <TouchableOpacity
                                ...
                                className={`flex-1 min-w-[45%] p-3 rounded-xl ${preferences.fab.iconType === 'custom'
                                    ? 'bg-indigo-50 dark:bg-indigo-500/20 border-2 border-indigo-500'
                                    : 'bg-white dark:bg-black border border-gray-200 dark:border-zinc-800'
                                    }`}
                            >
                                <View className="flex-row items-center">
                                    ...
                                    <Typography className={`ml-2 text-sm font-semibold ${preferences.fab.iconType === 'custom' ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-600 dark:text-gray-400'
                                        }`}>
                        ...
                        {/* Icon Color */}
                        <Typography className="text-base font-bold text-gray-900 dark:text-gray-100 mb-3">{t.superAssistant.iconColor}</Typography>
                        <View className="flex-row flex-wrap gap-2 mb-4">
                            {PRESET_COLORS.map((color) => (
                                <TouchableOpacity
                                    ...
                                    style={{
                                        ...
                                        borderColor: preferences.fab.iconColor === color.value ? color.value : 'transparent',
                                    }}
                                >
                                    <View
                        ...
                        <Typography className="text-base font-bold text-gray-900 dark:text-gray-100 mb-3">{t.superAssistant.animationMode || 'Animation Mode'}</Typography>
                            ...
                            <View className="flex-row gap-3">
                                {ANIMATION_MODES.map((mode) => (
                                    <TouchableOpacity
                                        ...
                                        className={`px-4 py-3 rounded-xl items-center justify-center min-w-[100px] ${preferences.fab.animationMode === mode.mode
                                            ? 'bg-indigo-50 dark:bg-indigo-500/20 border-2 border-indigo-500'
                                            : 'bg-white dark:bg-black border border-gray-200 dark:border-zinc-800'
                                            }`}
                                    >
                                        {(LucideIcons as any)[mode.icon] && React.createElement((LucideIcons as any)[mode.icon], {
                                            size: 20,
                                            color: preferences.fab.animationMode === mode.mode ? '#6366f1' : (isDark ? '#9ca3af' : '#6b7280')
                                        })}
                                        <Typography className={`mt-2 text-xs font-bold ${preferences.fab.animationMode === mode.mode ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-600 dark:text-gray-400'
                                            }`}>
                                            {mode.label}
                                        </Typography>
                                    </TouchableOpacity>
