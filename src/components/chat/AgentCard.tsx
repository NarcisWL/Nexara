...
export const AgentCard = ({ agent, onPress, onLongPress, style }: AgentCardProps) => {
  ...
  const IconComponent =
    (LucideIcons as any)[agent.avatar || 'MessageSquare'] || LucideIcons.MessageSquare;
  ...
  return (
    ...
    >
      <View className="bg-gray-50 dark:bg-zinc-900 rounded-[28px] p-5 border border-gray-100 dark:border-zinc-800 shadow-sm">
        ...
        <View className="flex-row justify-between items-start mb-4">
          ...
          {agent.isPreset && (
            <View className="bg-indigo-50 dark:bg-indigo-500/10 px-2 py-0.5 rounded-full border border-indigo-100 dark:border-indigo-900/30">
              <Typography className="text-indigo-600 dark:text-indigo-400 font-bold text-[9px] uppercase tracking-tighter">
                PRESET
              </Typography>
            </View>
          )}
        ...
        <Typography
          variant="h3"
          className="text-gray-900 dark:text-white font-bold leading-tight mb-1"
          numberOfLines={1}
        >
          {agent.name}
        </Typography>
        <Typography
          variant="body"
          className="text-gray-500 dark:text-gray-400 text-[13px] leading-5"
          numberOfLines={3}
        >
          {agent.description}
        </Typography>
        ...
        <View className="mt-4 pt-3 border-t border-gray-100 dark:border-zinc-800/50 flex-row items-center">
          ...
          <Typography className="text-gray-400 font-bold text-[10px] uppercase tracking-widest">
            {agent.defaultModel}
          </Typography>
        </View>
      </View>
