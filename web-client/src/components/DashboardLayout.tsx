...
export function DashboardLayout() {
    ...
    const mainNav = [
        ...
        { icon: Book, label: t.sidebar.library, path: '/library' },
    ...
    return (
        <div className="flex h-screen bg-gray-50 dark:bg-gray-900 overflow-hidden">
            ...
            <aside className={clsx(
                ...
                {/* Scrollable Content */}
                <div
                    className="flex-1 overflow-y-auto py-6 px-4 space-y-8 [&::-webkit-scrollbar]:hidden"
                    style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}
                >

                    {/* Main Nav */}
                    <div className="space-y-1">
                        <div className="px-3 text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-2">{t.sidebar.menu}</div>
                        {mainNav.map(item => (
                            <Link
                                key={item.path}
                                to={item.path}
                        ...
                        <div className="space-y-4">
                            ...
                            {sessionsByAgent['unknown'] && sessionsByAgent['unknown'].length > 0 && (
                                <div className="pt-4 border-t border-white/5 mt-4">
                                    <h3 className="px-3 text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-2">{t.sidebar.unassigned}</h3>
                ...
                <div className="p-4 border-t border-[rgba(255,255,255,0.08)] bg-[#09090b]/40">
                    <div className="flex items-center gap-2 px-3 py-2 bg-zinc-900/50 border border-zinc-800/50 rounded-lg mb-3">
                        ...
                        <span className="text-xs font-medium text-zinc-400 capitalize">
                            {connectionStatus === 'authenticated' ? t.sidebar.systemOnline : connectionStatus}
                        </span>
                    </div>
