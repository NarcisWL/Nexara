...
export class EmbeddingClient {
     ...
     */
    private async embedVertexAI(texts: string[]): Promise<{ embeddings: number[][]; usage?: { total_tokens: number } }> {
        ...
        for (let i = 0; i < texts.length; i += batchSize) {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Vertex AI Embedding failed: ${response.status} ${errorText.substring(0, 200)}`);
            }
     ...
     */
    private async embedGemini(texts: string[]): Promise<{ embeddings: number[][]; usage?: { total_tokens: number } }> {
        ...
        for (const text of texts) {
            ...
            if (!response.ok) {
                const errorText = await response.text();
                throw new Error(`Gemini Embedding failed: ${response.status} ${errorText.substring(0, 200)}`);
            }
    ...
    private async embedOpenAI(texts: string[]): Promise<{ embeddings: number[][]; usage?: { total_tokens: number } }> {
        ...
        for (let i = 0; i < texts.length; i += batchSize) {
            ...
            if (!response.ok) {
                const errorText = await response.text();
                throw new Error(`OpenAI Embedding failed: ${response.status} ${errorText.substring(0, 200)}`);
            }
