    ...
    return (
        <View className="flex-1">
            ...
            {providers.length === 0 ? (
                ...
                providers.map((p) => (
                    <View key={p.id} className="bg-white dark:bg-zinc-900 rounded-2xl p-4 mb-3 border border-gray-100 dark:border-zinc-800 shadow-sm">
                        <View className="flex-row justify-between items-start">
                            <View>
                                <Typography className="font-bold text-lg">{p.name}</Typography>
                                <Typography className="text-gray-400 text-xs uppercase tracking-tighter">{p.type} • {p.enabled ? '已启用' : '已禁用'}</Typography>
                            </View>
            ...
            <Modal visible={isAddModalOpen} animationType="fade" transparent={true}>
                <View className="flex-1 justify-center items-center bg-black/50 p-6">
                    <View className="bg-white dark:bg-zinc-900 rounded-3xl p-6 w-full">
                        <Typography variant="h2" className="mb-4">{editingProvider ? '编辑供应商' : '添加供应商'}</Typography>
                            ...
                            <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mb-4">
                                {(['openai', 'google', 'deepseek', 'anthropic', 'moonshot', 'zhipu', 'siliconflow', 'github'] as ApiProviderType[]).map(t => (
                                    <TouchableOpacity
                                        ...
                                        className={clsx("px-4 py-2 rounded-full mr-2", formData.type === t ? "bg-indigo-600" : "bg-gray-100 dark:bg-zinc-800")}
                                    >
                                        <Text className={clsx("font-bold text-xs", formData.type === t ? "text-white" : "text-gray-400")}>{t.toUpperCase()}</Text>
                                    </TouchableOpacity>
                                ))}
                            </ScrollView>
                            ...
                            {formData.type === 'google' ? (
                                <View>
                                    ...
                                    >
                                        ...
                                        <Typography className="text-indigo-600 font-bold">导入 Service Account JSON</Typography>
                                    </TouchableOpacity>
                                    {formData.vertexProject && (
                                        <Typography className="text-xs text-green-500 mb-4 text-center">✅ 已识别项目: {formData.vertexProject}</Typography>
                                    )}
                                </View>
                            ) : (
                                <View>
                                    ...
                                    <TextInput
                                        ...
                                        placeholder="sk-..."
                                    />
                                    <Typography className="mb-1 text-xs font-bold text-gray-400">Base URL (可选)</Typography>
                                    <TextInput
                        ...
                        <View className="flex-row mt-4">
                            ...
                            <TouchableOpacity onPress={handleSave} className="flex-1 py-3 bg-indigo-600 rounded-xl items-center">
