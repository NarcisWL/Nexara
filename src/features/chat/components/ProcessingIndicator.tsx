...
export const ProcessingIndicatorChip: React.FC<ProcessingIndicatorChipProps> = ({
  ...
  const isActiveProcessing =
    ...
    messageId === processingState.activeMessageId;
  ...
  const darkBaseColor = showCompleted
    ? completedColorDark
    : status === 'completed'
      ? '#34d399'
      : '#38bdf8';
  ...
  const borderColor =
    expanded || showCompleted
      ? isDark
        ? `rgba(${showCompleted ? (isSummarized ? '96, 165, 250' : '52, 211, 153') : '52, 211, 153'}, 0.4)`
        : baseColor
      : isDark
    ...
    return (
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
        ...
        {messageHistory?.type === 'archived' || isSummarized ? (
          <TouchableOpacity activeOpacity={0.7} onPress={onToggle} style={{ padding: 8 }}>
  ...
  return (
    ...
    >
      <View style={styles.header}>
        ...
        <Text style={[styles.statusText, { color: textColor }]}>{getStatusText()}</Text>
        {expanded ? (
 ...
 */
export const ProcessingIndicatorDetails: React.FC<ProcessingIndicatorDetailsProps> = ({
  isDark,
  status,
  messageId,
}) => {
    ...
    >
      ...
      {summary && (
        ...
        >
          ...
          <Text style={[styles.chunkContent, { color: textColor }]}>{summary}</Text>
        </View>
      )}
