...
export async function performWebSearch(query: string, apiKey?: string, cx?: string): Promise<{ context: string; sources: SearchResult[] }> {
    ...
    try {
        ...
        console.log(`Searching Google: ${query} (Key: ${cleanApiKey.substring(0, 5)}... length=${cleanApiKey.length})`);
...
function getMockResults(query: string, isFallback = false): { context: string; sources: SearchResult[] } {
