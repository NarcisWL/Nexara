...
export class GraphExtractor {
    ...
    private getClient(modelId?: string) {
        ...
        let targetId = modelId || settingsState.globalRagConfig.kgExtractionModel || settingsState.defaultSummaryModel;
    ...
    private getSystemPrompt(): string {
        ...
        const entityTypes = settingsState.globalRagConfig.kgEntityTypes || ['Concept', 'Person', 'Organization', 'Location', 'Event', 'Product'];
    ...
    async extractAndSave(text: string, docId?: string): Promise<ExtractionResult> {
        try {
            ...
            console.log(`[GraphExtractor] Extracted ${result.nodes.length} nodes and ${result.edges.length} edges`);
            ...
            for (const edge of result.edges) {
                ...
                if (sourceId && targetId) {
                    try {
                        await graphStore.createEdge(sourceId, targetId, edge.relation, docId, edge.weight || 1.0);
                    } catch (e) {
                        console.error(`[GraphExtractor] Failed to save edge ${edge.source}->${edge.target}`, e);
                    }
                } else {
                    console.warn(`[GraphExtractor] Skipping edge: missing node ID for ${edge.source} or ${edge.target}`);
                }
            }
