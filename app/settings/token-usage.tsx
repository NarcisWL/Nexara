    ...
    const ModelRow = ({ modelId, usage }: { modelId: string, usage: BillingUsage }) => (
        <View style={{
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                <Typography className="font-bold text-base text-black dark:text-white">{modelId}</Typography>
            ...
            >
                ...
                <View style={{ alignItems: 'center', marginBottom: 32 }}>
                    <View style={{
                        ...
                        borderWidth: 4, borderColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.05)',
                        borderStyle: 'dashed',
                        alignItems: 'center', justifyContent: 'center',
                        marginBottom: 24
                    }}>
                        <Typography className="text-5xl font-black text-black dark:text-white" style={{ letterSpacing: -2 }}>
                            {(globalTotal.total / 1000).toFixed(1)}k
                        </Typography>
                        <Typography className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest mt-2">Total Tokens</Typography>
                    </View>

                    <View style={{ flexDirection: 'row', gap: 12, width: '100%' }}>
                        <MetricCard label="Input" metric={globalTotal.chatInput} color="#8b5cf6" icon={MessageSquare} />
                        <MetricCard label="Output" metric={globalTotal.chatOutput} color="#f59e0b" icon={Zap} />
                        <MetricCard label="System" metric={globalTotal.ragSystem} color="#10b981" icon={Database} />
                    </View>
                </View>
                ...
                <View style={{ marginBottom: 40 }}>
                    ...
                    {Object.keys(byModel).length === 0 && (
                        <Typography className="text-gray-400 text-center py-8">No usage history found.</Typography>
                    )}
                </View>
                ...
                >
                    ...
                    <Typography className="font-bold text-red-500 dark:text-red-400">Reset All Statistics</Typography>
                </TouchableOpacity>
