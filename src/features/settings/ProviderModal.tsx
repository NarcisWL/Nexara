...
const PROVIDER_PRESETS: Record<string, { name: string; baseUrl: string; type: ApiProviderType }> = {
    ...
    qwen: { name: '通义千问 (Qwen)', baseUrl: 'https://dashscope.aliyuncs.com/compatible-mode/v1', type: 'openai' },
    ernie: { name: '文心一言 (ERNIE)', baseUrl: 'https://aip.baidubce.com/rpc/2.0/ai_custom/v1', type: 'openai' },
    doubao: { name: '豆包 (Doubao)', baseUrl: 'https://ark.cn-beijing.volces.com/api/v3', type: 'openai' },
...
export function ProviderModal({ visible, onClose, onSave, editingProvider }: ProviderModalProps) {
    ...
    const [errors, setErrors] = useState<{ name?: string; apiKey?: string; vertexProject?: string; jsonInput?: string }>({});
    ...
    const handleSave = () => {
        const newErrors: { name?: string; apiKey?: string; vertexProject?: string; jsonInput?: string } = {};
    ...
    return (
        ...
        >
            ...
            >
                ...
                >
                    ...
                    {!editingProvider && (
                        <View style={{ marginBottom: 20 }}>
                            <Text style={{ fontSize: 13, fontWeight: 'bold', color: '#9ca3af', marginBottom: 10 }}>
                            ...
                            >
                                <Text style={{ fontSize: 14, color: selectedPreset ? (isDark ? '#fff' : '#111') : '#9ca3af' }}>
                                ...
                                <View style={{
                                    ...
                                    >
                                        {Object.entries(PROVIDER_PRESETS).map(([key, preset]) => (
                                            ...
                                            >
                                                <Text style={{ fontSize: 14, fontWeight: selectedPreset === key ? '600' : '400', color: selectedPreset === key ? '#6366f1' : (isDark ? '#fff' : '#111') }}>
                                                    {preset.name}
                                                </Text>
                                            </TouchableOpacity>
                                        ))}
                    ...
                    <View style={{ gap: 16 }}>
                        {/* 名称 */}
                        <View>
                            <Text style={{ fontSize: 14, fontWeight: '600', color: isDark ? '#fff' : '#111', marginBottom: 8 }}>
                                ...
                                <View>
                                    ...
                                    <TextInput
                                        ...
                                        style={{
                                            backgroundColor: isDark ? '#18181b' : '#f9fafb',
                                            borderWidth: 1,
                                            borderColor: errors.vertexProject ? '#ef4444' : (isDark ? '#27272a' : '#e5e7eb'),
                                            borderRadius: 12,
                                            paddingHorizontal: 16,
                                            paddingVertical: 10,
                                            fontSize: 16,
                                    ...
                                    {errors.vertexProject && (
                                        <Text style={{ color: '#ef4444', fontSize: 12, marginTop: 4 }}>{errors.vertexProject}</Text>
                                    )}
                                </View>
                        ...
                        {type === 'google' && (
                            <View>
                                ...
                                />
                                {errors.jsonInput && (
                                    <Text style={{ color: '#ef4444', fontSize: 12, marginTop: 4 }}>{errors.jsonInput}</Text>
                                )}
                            </View>
                        )}
                    </View>
                ...
                {/* Footer Buttons */}
                <View style={{ paddingHorizontal: 24, paddingVertical: 16, borderTopWidth: 1, borderTopColor: isDark ? '#27272a' : '#e5e7eb', gap: 12 }}>
                    <TouchableOpacity
                        onPress={handleSave}
                        style={{
