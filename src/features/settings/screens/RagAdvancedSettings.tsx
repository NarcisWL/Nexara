...
import { View, TextInput, ScrollView, TouchableOpacity, Alert, KeyboardAvoidingView, Platform, Modal, FlatList } from 'react-native';
...
import { Network, Cpu, DollarSign, Edit3, Save, ChevronLeft, Bot, Check, X } from 'lucide-react-native'; import { useApiStore } from '../../../store/api-store';
...
const SectionHeader: React.FC<{ title: string; mt?: number }> = ({ title, mt = 24 }) => {
    ...
    return (
        <View style={{ marginTop: mt, marginBottom: 16 }}>
            <Typography style={{ fontSize: 13, fontWeight: 'bold', color: isDark ? Colors.dark.textSecondary : Colors.light.textSecondary, textTransform: 'uppercase', letterSpacing: 0.5 }}>
                {title}
            </Typography>
        </View>
    );
                ...
                >
                        ...
                        <View className="p-4 flex-row justify-between items-center">
                            <View style={{ flex: 1 }}>
                                <View className="flex-row items-center mb-1">
                                    ...
                                    <Typography className="font-bold text-gray-900 dark:text-gray-100">启用知识图谱</Typography>
                                </View>
                                <Typography className="text-xs text-gray-500">在向量化时自动抽取实体与关系 (需配置 LLM)</Typography>
                            </View>
                            <Switch
                        ...
                        <View>
                            ...
                            >
                                <View style={{ flex: 1, opacity: globalRagConfig.enableKnowledgeGraph ? 1 : 0.5 }}>
                                    <View className="flex-row items-center mb-1">
                                        <Bot size={16} color={globalRagConfig.enableKnowledgeGraph ? "#10b981" : "#94a3b8"} style={{ marginRight: 6 }} />
                    ...
                    <View className="bg-white dark:bg-zinc-900 rounded-2xl p-4 border border-gray-100 dark:border-zinc-800 mb-6">
                        <View className="flex-row items-center mb-4">
                            ...
                            <View style={{ flex: 1 }}>
                                <Typography className="text-base font-bold text-gray-900 dark:text-white">抽取策略</Typography>
                                <Typography className="text-xs text-gray-500">决定何时以及如何调用 LLM 进行知识抽取</Typography>
                            </View>
                        </View>

                        {/* Radio Options */}
                        {[
                            { key: 'summary-first', label: '摘要优先 (Summary First)', desc: '仅分析文档摘要，成本最低 (推荐)', color: '#10b981' },
                            { key: 'on-demand', label: '按需深挖 (On-Demand)', desc: '仅手动触发详细抽取', color: '#f59e0b' },
                            { key: 'full', label: '全量扫描 (Full Scan)', desc: '分析每个片段，成本极高', color: '#ef4444' }
                        ].map((opt) => (
                            <TouchableOpacity
                                key={opt.key}
                                ...
                                style={{ borderColor: globalRagConfig.costStrategy === opt.key ? opt.color : 'transparent' }}
                            >
                                <View className="flex-row items-center">
                                    <View style={{ width: 16, height: 16, borderRadius: 8, borderWidth: 2, borderColor: opt.color, alignItems: 'center', justifyContent: 'center', marginRight: 10 }}>
                                        {globalRagConfig.costStrategy === opt.key && <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: opt.color }} />}
                                    </View>
                                    <View>
                                        <Typography className="font-bold text-sm text-gray-800 dark:text-gray-200">{opt.label}</Typography>
                        ...
                        <View className="p-4 border-b border-gray-100 dark:border-gray-800 flex-row justify-between items-center">
                            <View style={{ flex: 1 }}>
                                <View className="flex-row items-center mb-1">
                                    ...
                                    <Typography className="font-bold text-gray-900 dark:text-gray-100">增量哈希校验</Typography>
                                </View>
                                <Typography className="text-xs text-gray-500">跳过内容未变更的文档，避免重复抽取</Typography>
                            </View>
                            <Switch
                        ...
                        <View className="p-4 flex-row justify-between items-center">
                            <View style={{ flex: 1 }}>
                                <View className="flex-row items-center mb-1">
                                    ...
                                    <Typography className="font-bold text-gray-900 dark:text-gray-100">规则预筛选</Typography>
                                </View>
                                <Typography className="text-xs text-gray-500">使用正则过滤低价值文本，不发送给 LLM</Typography>
                            </View>
                            <Switch
                    ...
                    <View className="bg-white dark:bg-zinc-900 rounded-2xl p-4 border border-gray-100 dark:border-zinc-800">
                        <View className="flex-row justify-between items-center mb-4">
                            <Typography className="text-sm font-bold text-gray-900 dark:text-gray-100">实体关系抽取 Prompt</Typography>
                            <View style={{ flexDirection: 'row', gap: 12 }}>
                                ...
                                <TouchableOpacity onPress={handleSavePrompt} style={{ flexDirection: 'row', alignItems: 'center' }}>
...
const ModelSelectionModal: React.FC<{
    ...
    const allModels = useMemo(() => {
        ...
        providers.forEach(p => {
            if (p.enabled) {
                p.models.forEach(m => {
                    if (m.enabled) { // Only enabled models
        ...
        <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
            <View className="flex-1 bg-black/50 justify-center px-6">
                <View className="bg-white dark:bg-zinc-900 rounded-3xl max-h-[70%] overflow-hidden">
                    <View className="p-4 border-b border-gray-100 dark:border-gray-800 flex-row justify-between items-center">
                        <Typography className="font-bold text-lg text-gray-900 dark:text-white">选择模型</Typography>
                    ...
                    <ScrollView className="p-2">
                        ...
                        >
                            <View>
                                <Typography className="font-bold text-gray-900 dark:text-white">跟随系统默认</Typography>
                        ...
                        {allModels.map((model) => (
                            ...
                            >
                                <View>
                                    <Typography className="font-bold text-gray-900 dark:text-white">{model.name}</Typography>
